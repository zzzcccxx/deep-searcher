...
"""
    ...
    response_prompt = f"""Based on the above, please analyze whether the content of these small questions is related to the main question and whether it can help answer the main question.The output content is a json.
...
according to the related content, all related questions and content are first analyzed to see if they are related to the question. The judgment result is stored in the first-level key of the json. The key is **related_content**, and the value is a sub-json. The key of this sub-json is the question, and the value is YES or NO. If the question and content are helpful in answering the main question, it is YES, otherwise NO. 
...
def get_final_answer_prompt(
   ...
   mini_chuncks: List[str],
    ...
    sections = []
    common_prompt = f"""You are a AI content analysis expert, good at summarizing content. After many attempts, we have obtained the answers to the following questions. Please summarize a simple and clear answer based on the following series of small questions and their corresponding contents.
...
""")
    data_set_prompt = f"""The following is a series of small questions and their corresponding content.

Small Questions And Answers List:
{"\n".join(data_set)}
    """
    sections.append(data_set_prompt)
